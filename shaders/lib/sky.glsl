...
vec3 analyticSky(vec3 rd, vec3 sunDir, float rain, float time01) {
    ...
    vec3 zenith = mix(vec3(0.015, 0.025, 0.07), vec3(0.15, 0.38, 0.72), sunHeight);
    vec3 horizonColor = mix(vec3(0.025, 0.035, 0.08), vec3(0.66, 0.78, 0.94), sunHeight);
    horizonColor = mix(horizonColor, vec3(1.0, 0.30, 0.08), dusk * 0.82);
